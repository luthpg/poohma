...
3. **ゲストkick**: ゲストの `familyId` を `undefined` にパッチ（メール送信はスキップ）。
4. **申請保護クリーンアップ**: `joinRequests` のうち作成から 48 時間以上経過した申請のみを削除（直近の申請者を保護）。
5. **初期データ再投入**: `demoRecords.json`（E2Eシード由来の暗号化済み実データ）からレコード・クレデンシャルを一括再作成。
6. **無期限招待維持**: 招待コードの有効期限を 2099 年に延長。
7. **実効的防御策**: 直近 10 分以内の連続実行はクールダウンガードによりスキップ。
