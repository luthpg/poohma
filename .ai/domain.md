...
flowchart TD
    ...
    Pending -->|"ファミリー管理者が承認 (approveJoinRequest)"| Approved["approved<br/>(user.familyId 更新)"]
    Pending -->|"ファミリー管理者が拒否 / 申請者が取り下げ"| Rejected["rejected<br/>(アクセス権なし)"]
...
- **承認時**: ファミリー管理者（`familyAdminMutation`）が承認（`approveJoinRequest`）すると、対象アカウントの `user.familyId` が更新され、申請者に通知（一般メンバーは一覧確認のみ可能）。
- **拒否・取り下げ時**: ファミリー管理者による拒否（`rejectJoinRequest`）または申請者自身によるキャンセル（`cancelJoinRequest`）により `rejected` となり、家族へのアクセス権は付与されない。
...
2. **完全消去（ゲスト個人レコード＋共有レコード）**:
   ...
   - ゲストが作成した個人所有レコード（`ownerType === "user"`）もすべて一時データとして完全消去。
   - `pendingExportVaults` は退避せず、ゾンビデータを残さない。
3. **閲覧ログ・移行データのクリーンアップ（コホート間漏洩防止）**:
   - `viewLogs`（`familyId === DEMO_FAMILY_ID`）をファミリー単位で一括パージ。
   - ゲストの `familyMigrations`（`sourceFamilyId` or `targetFamilyId === DEMO_FAMILY_ID`）を全削除。
4. **ゲストkick**: ゲストの `familyId` を `undefined` にパッチ（メール送信はスキップ）。
5. **参加申請のクリーンアップ**:
   - `approved` / `rejected`: 判定済みなので無条件で全削除。`approved` 放置はキック後のロックインバグを引き起こすため必須。
   - `pending`: 直近の申請者保護のため、48 時間経過分のみ削除。
6. **監査ログの全パージ**: `auditLogs`（`familyId === DEMO_FAMILY_ID`）を全件パージ。パージ後にリセット完了ログ 1 件のみ新規記録（クールダウンガード判定に必要）。
7. **初期データ再投入**: `demoRecords.json`（E2Eシード由来の暗号化済み実データ）からレコード・クレデンシャルを一括再作成。
