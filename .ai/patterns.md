...
- **デッドロック防止**: 編集セッションは他者の編集をブロックせず、「○○さんが編集中です」という警告のみを表示する。ブラウザ強制終了や完全離脱時も5分で自然失効するため、レコードがロックされて家族が編集不能になる事故を防ぐ。

### 2. 楽観的ロック（Hard Invariant: `updateRecord`）

- **タイムスタンプ突合**: クライアントは編集開始時に保持した `updatedAt` を渡して保存する。DBの現在の `record.updatedAt` と一致しない場合、`CONFLICT` エラーを送出し、先行保存データを無警告で上書きする事故を防ぐ。
