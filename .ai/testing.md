...
## 9. コミット前の動的テスト（E2E / `pnpm test:e2e`）によるローカル検証義務

AI Agent がコード変更を行う際、静的検証（`pnpm check` や `pnpm typecheck`）および Vitest ユニットテスト（`pnpm test`）のみで作業を完了とし、コミット・プッシュしてはならない。

- **義務の対象**:
  - UI 挙動の変更、新規機能・導線の追加
  - 認証・認可・セッション制御
  - E2EE 暗号化・鍵導出・ストレージ処理
