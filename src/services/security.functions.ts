...
/**
 * 家族パスコード変更 Server Function
 */
export const rotatePasscodeServerFn = createServerFn({ method: "POST" })
  .validator(
    (data: {
  ...
  .handler(async ({ data }) => {
    ...
    return await client.mutation(api.families.rotatePasscode, {
...
/**
 * リカバリーキー発行・再発行 Server Function
 */
export const issueRecoveryKeyServerFn = createServerFn({ method: "POST" })
  .validator(
    (data: {
      masterKeyRecoveryEncrypted: string;
  ...
  .handler(async ({ data }) => {
    ...
    return await client.mutation(api.families.issueRecoveryKey, {
      masterKeyRecoveryEncrypted: data.masterKeyRecoveryEncrypted,
      masterKeyRecoveryIv: data.masterKeyRecoveryIv,
      accountId: data.accountId as Id<"users"> | undefined,
      ...context,
    });
  });
...
/**
 * リカバリーキーを使用したパスコード再設定・復旧 Server Function
 */
export const recoverWithRecoveryKeyServerFn = createServerFn({
  method: "POST",
})
  .validator(
    (data: {
  ...
  .handler(async ({ data }) => {
    ...
    return await client.mutation(api.families.recoverWithRecoveryKey, {
      familyId: data.familyId as Id<"families">,
      masterKeyEncrypted: data.masterKeyEncrypted,
      masterKeyIv: data.masterKeyIv,
      masterKeySalt: data.masterKeySalt,
      accountId: data.accountId as Id<"users"> | undefined,
