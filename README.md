...
- [PoohMaとは](#poohmaとは)
- [主な特徴](#主な特徴)
- [デモ環境でお試し（Live Demo）](#デモ環境でお試しlive-demo)
- [技術スタック](#技術スタック)
...
より詳しい機能一覧は [機能一覧 (.docs/features.md)](.docs/features.md) および [要件定義書](#関連ドキュメント) を参照してください。

## デモ環境でお試し（Live Demo）

ポートフォリオ閲覧や技術検証用に、PoohMa の「ゼロ知識E2EE（エンドツーエンド暗号化）」および「家族承認フロー」を実際に体験できる公開デモファミリーを用意しています。

- **URL**: [https://poohma.ciderlabs.link](https://poohma.ciderlabs.link)
- **デモ招待コード**: `5f76e73f-ea8b-485f-900b-ffc61a69d83b`
- **デモ家族パスコード**: `Demo5917@p`

> [!NOTE]
> **ゼロ知識暗号化とセキュリティ設計について**
> - **パスコード公開の理由**: デモデータは架空のサブスクリプション情報のみで構成されており、毎日午前4:00 (JST) に GitHub Actions によって初期状態へ自動リセットされます。
> - **サーバー不保持の証明**: 家族パスコードはブラウザ側（Web Crypto API）でのみ暗号鍵の導出に使われ、サーバーへは一切送信されません（Zero-Knowledge Architecture）。
> - **家族承認制の実演**: 招待コードとパスコードを入力して申請後、ファミリー管理者（当方）による手動承認を経て初めて暗号化データが復号・表示されます。第三者の勝手な参加やデータ閲覧を防ぐPoohMaの認可境界を体感いただけます。

### 体験の手順

1. アプリにアクセスし、Google アカウントでログイン
2. 初回画面（または家族切り替え）で「既存の家族に参加する」を選択
3. 上記の **招待コード** と **パスコード** を入力して参加申請を送信
4. 管理者（当方）が申請を承認すると、暗号化されたデモレコード（架空の共有アカウント情報）が閲覧可能になります  
   *(※ 採用担当者様・選考関係者様はお気軽に申請いただくか、GitHub Issue / メッセージ等でお知らせいただければ即座に承認いたします)*
...
| [機能一覧](.docs/features.md) | 全提供機能の業務・画面・要件別カタログ |
...
| DEMO\_FAMILY\_ID                                               | 定期リセット対象のデモファミリーID（任意・デモ運用時必須） |
| DEMO\_ADMIN\_USER\_IDS                                         | デモファミリーの管理者ユーザーID（カンマ区切り、kick除外用。PoohMaアカウントID `_id` または Firebase UID のいずれでも可） |

| DEMO\_INVITE\_CODE                                             | デモファミリー用招待コード（任意、デフォルト: `poohma-demo`） |

GitHub Actions シークレット（デモ自動リセットワークフロー用）：

| シークレット名                   | 必須 | 説明                                                |
| ----------------------------- | -- | ------------------------------------------------- |
| CONVEX\_SITE\_URL             | 必須 | Convex HTTPアクションの公開URL（例: `https://<deployment>.convex.site`） |
| CONVEX\_INTERNAL\_SECRET      | 必須 | `POST /resetDemoFamily` 認証用シークレット（上記Convex環境変数と一致） |
| DISCORD\_WEBHOOK\_URL         | 任意 | リセット実行結果・失敗時の通知先Discord Webhook URL          |
