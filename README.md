...
- **マルチアカウント・マルチファミリー対応** ：1つのログイン（Firebase Auth）で用途や所属家族に応じた複数のPoohMaアカウントを作成・安全に切り替え可能。非公開（個人用）レコードを含め、すべてのデータ・所属ファミリー・E2EE暗号化境界がアカウント単位で完全に分離されます。
...
| ドキュメント | 内容                           |
| ------ | ---------------------------- |
| 要件定義書  | 機能要件・非機能要件、画面一覧、制約事項         |
| 設計書    | アーキテクチャ、DBスキーマ、API設計、主要シーケンス |
