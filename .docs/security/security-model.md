...
- 未保存ドラフト保護（FR-REC-28）：iOS Safari 等のバックグラウンドメモリ解放（Jetsam）やサイレント再認証後の入力復元のため、未保存フォーム値は短命 `draftDEK` でクライアント暗号化（`masterKey` でラップ）され、`localStorage` に退避される。平文や未ラップの鍵材料はストレージに一切保存されず、24時間TTL、アカウント切り替え・ログアウト時の即時物理破棄、別タブ複製時の `BroadcastChannel` による衝突自律回避を行うことで、E2EE境界およびアカウント分離境界を厳格に維持する。
