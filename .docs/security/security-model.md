...
- サーバー内部のみで完結すべき通信（`getUserByFirebaseUid`）は、共有シークレット（`CONVEX_INTERNAL_SECRET`）をヘッダーで検証する内部専用エンドポイントとして分離している。ヘッダーのシークレット照合には固定長（256文字）パディングによる定数時間比較（`timingSafeEqual`）を適用し、タイミング攻撃（CWE-208）によるシークレット文字内容および長さ情報の推測を防止している。
...
- リカバリーコードのハッシュ照合（`family.recoveryCodeHash`）、OTPコードのハッシュ照合（`otpRecord.codeHash`）、および認可セッショントークンのハッシュ照合（`sessionRecord.sessionTokenHash`）の全比較処理において、固定長（256文字）パディングによる定数時間比較（`timingSafeEqual`）を徹底し、サイドチャネル攻撃によるハッシュ値推測を防止している。
...
- 未保存ドラフト保護（FR-REC-28）：iOS Safari 等のバックグラウンドメモリ解放（Jetsam）やサイレント再認証後の入力復元のため、未保存フォーム内の機密情報（パスワードヒント）は短命 `draftDEK` でクライアント暗号化（`masterKey` でラップ）され、平文メタデータとともに `localStorage` に退避される。平文ヒントや未ラップの鍵材料はストレージに一切保存されず、24時間TTL、非nullの `accountId` 厳格照合によるアカウント間分離、明示的キャンセル・保存成功・競合解決時の即時物理破棄、別タブ複製時の `BroadcastChannel` による衝突自律回避を行うことで、E2EE境界およびアカウント分離境界を厳格に維持する。
...
- 招待コードはあくまで「参加申請を送信する権利」であり、正式な家族参加にはファミリー管理者による明示的な承認（`joinRequests` の approve）が必須の二段階構成になっている（参加申請の承認・却下はファミリー管理者に限定され、一般メンバーは一覧確認のみ可能）。
